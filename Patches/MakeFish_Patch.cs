...
namespace LuckyFishing.Patches;

[HarmonyPatch(typeof(AI_Fish), nameof(AI_Fish.Makefish))]
internal static class MakeFish_Patch {
    ...
    public static CodeInstructions Transpiler(CodeInstructions instructions, MethodBase method) {
        ...
        if (LuckyFishing.Config!.TryGetEntry<bool>("General", "AffectJunkChance", out var affectJunk) && affectJunk.Value) {
            editor.Replace([
                Constant(5),
                LocalVariable(0),
                Constant(3),
